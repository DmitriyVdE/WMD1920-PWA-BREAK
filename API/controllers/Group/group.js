...
exports.create_group = async function (req, res) {
  const temp = req.body;
  if (
    !(await checkUserIdExists(temp.userId)) ||
    temp.groupName == "" ||
    temp.password == ""
  ) {
    res.status(400).send({
      error:
        "Please make sure you add a valid 'userId', 'groupName' and 'password'.",
    });
    return;
  }
...
exports.get_group_info = async function (req, res) {
  if (await checkGroupCodeExists(req.params.groupId)) {
    Group.findOne({ groupCode: req.params.groupId }, async function (
      ...
      io.to(req.params.groupId).emit("join", "User joined");
      res.json({
    ...
    res.status(400).send({
      error: "Please make sure you are trying to join an existing group.",
    });
    return;
  }
};
...
exports.add_group_owner = async function (req, res) {
  if (
    ...
    (await groupPasswordIsCorrect(req.body.groupCode, req.body.password))
  ) {
    ...
    res
      .status(400)
      .send({ error: "Group does not exist or incorrect password." });
    return;
  }
};
...
exports.update_group_info = async function (req, res) {
  if (
    ...
    Group.findOne({ groupCode: req.body.groupCode }, function (err, group) {
      ...
      io.to(req.params.groupId).emit("info", "Groupinfo updated");
...
exports.delete_group = async function (req, res) {
  if (
    ...
    Group.findOne({ groupCode: req.body.groupCode }, function (err, group) {
      ...
      io.to(req.params.groupId).emit("delete", "Group has been deleted");
      res.json({ message: "The group " + groupName + " has been deleted." });
    });
  } else {
    res
      .status(400)
      .send({ error: "Group does not exist or password is incorrect." });
    return;
  }
};
...
exports.delete_group_users = async function (req, res) {
  if (
    ...
    Group.findOne({ groupCode: req.body.groupCode }, function (err, group) {
      ...
      io.to(req.params.groupId).emit("kick", "All non-owners have been kicked");
